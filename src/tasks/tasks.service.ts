...
const getAllTasks = async (): Promise<iTask[]> => {
  const allTasks = await getAllTasksDB().catch((err) => {
    throw err;
  });
...
const getTask = async (id: number): Promise<iTask> => {
  const task = await getTaskById(id).catch((err) => {
    throw err;
  });
...
const updateTask = async (id: number, title: string, description: string): Promise<iTask> => {
  const updTask = await updateTaskById(id, title, description).catch((err) => {
    throw err;
  });
...
const deleteTask = async (id: number): Promise<iTask> => {
  const delTask = await deleteTaskById(id).catch((err) => {
    throw err;
  });
...
const createTask = async (title: string, description: string): Promise<iTask> => {
  const newTask = await createNewTask(title, description).catch((err) => {
    throw err;
  });
